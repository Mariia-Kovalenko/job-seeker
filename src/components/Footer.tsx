    ...
    <footer className="border-t border-gray-200 mt-20">
      <div className="max-w-7xl mx-auto px-6 py-12 flex flex-col md:flex-row justify-between items-start gap-8 md:gap-0">
        ...
        <div className="flex flex-col gap-2">
          ...
          <Link to="/" className=" hover:text-orange-500 transition-colors opacity-80">
            Home
          </Link>
          <Link to="/jobs" className="hover:text-orange-500 transition-colors opacity-80">
            Jobs
          </Link>
        </div>
        ...
        {/* Categories */}
        <div className="flex flex-col gap-2">
          <h4 className="font-semibold ">Jobs Categories</h4>
          <div className="flex flex-col gap-1 mt-1">
            {CATEGORIES.map((cat, i) => (
              <Link to={`/jobs?category=${cat.name}`} key={i} className="opacity-80">
                {cat.name}
              </Link>
            ))}
          </div>
      ...
      <div className="border-t border-gray-200 mt-6 py-4">
        <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-4">
          <p className="opacity-80 text-sm">&copy; {new Date().getFullYear()} Job Seeker. All rights reserved.</p>
