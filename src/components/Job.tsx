...
export default function JobComponent({ job }: { job: Job }) {
    ...
    return (
        <a
            href={`/jobs/${job._id}`}
            key={job._id}
            className={`block p-3 text-white rounded-xl  border mb-4 md:mb-8 hover:shadow-xl transition-all duration-300 ${
                theme == "dark"
        ...
        >
            ...
            >
                ...
                <span className="flex gap-1 items-center whitespace-nowrap opacity-80">
                    {theme == "dark" ? (
                        <LocationIconLight />
                    ) : (
                        <svg
